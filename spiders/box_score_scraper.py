...
def scrape_day(month, day, year):
  ...
  if soup is None:
    url = "https://www.basketball-reference.com/boxscores/?month={}&day={}&year={}".format(month[1:], day, year)
    soup = scraper.get_soup(url)
    if soup is None:
  ...
  if a_tags is None:
    print("No games on {}-{}-{}".format(year, month, day))
    return
  ...
  for a_tag in a_tags:
    ...
    if "boxscore" in href and a_tag.text == 'Box Score':
      ...
      pgdb.insert(bs, table="games")
      pbp = scrape_pbp(base + "/boxscores/pbp/" + href.split("/")[-1])
      pgdb.insert(pbp, table="pbp")
